...
import { createQueryWrapper, userDetailResponse, userStatusRequest } from "@/test";
...
describe('useUpdateUserStatus', () => {
    it('USER-060 - Debe actualizar el estado del usuario correctamente', async () => {
        ...
        expect(vi.mocked(usersApi.updateStatus).mock.calls[0][1]).toEqual(userRequest)
    ...
    it('USER-063 - Debe invalidar la consulta del usuario con estado actualizado', async () => {
        ...
        // Assert
        expect(invalidateUserQuerySpy).toHaveBeenCalledWith({queryKey: ['user', 1]})
    })
})